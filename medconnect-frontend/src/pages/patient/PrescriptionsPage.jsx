...
const PrescriptionsPage = () => {
  ...
  const fetchPrescriptions = async () => {
    ...
    try {
      ...
      console.error(err)
      setError(err.response?.data?.message || 'Could not load prescriptions. Please try again.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div style={{ fontFamily: "'Segoe UI', 'Helvetica Neue', Arial, sans-serif", background: '#f0f4fa', minHeight: '100vh' }}>
      ...
      <nav style={{ background: '#1565c0', position: 'sticky', top: 0, zIndex: 50, boxShadow: '0 2px 12px rgba(21,101,192,0.18)' }}>
        ...
        {isMobile && mobileMenuOpen && (
          <div style={{ background: '#1565c0', padding: '12px 24px', display: 'flex', flexDirection: 'column', gap: 8, borderTop: '1px solid rgba(255,255,255,0.1)' }}>
            ...
            <button
              onClick={() => { navigate('/prescriptions'); setMobileMenuOpen(false); }}
              style={{ background: 'rgba(255,255,255,0.15)', border: 'none', color: '#fff', fontWeight: 700, fontSize: 15, padding: '10px 0', cursor: 'pointer', textAlign: 'left', width: '100%' }}
            >
              Prescriptions
            </button>
            <button
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', position: 'relative', zIndex: 2 }}>
          ...
          <h1 style={{ color: '#fff', fontSize: isMobile ? 28 : 36, fontWeight: 800, margin: '0 0 8px', letterSpacing: '-0.5px' }}>My Prescriptions</h1>
          <p style={{ color: '#bbdefb', fontSize: isMobile ? 14 : 15, margin: 0 }}>View, download and print your digital prescriptions</p>
        </div>
      </div>

      {/* Main Content */}
      <div style={{ maxWidth: 900, margin: '-40px auto 0', padding: isMobile ? '0 16px 60px' : '0 24px 60px', position: 'relative', zIndex: 10 }}>

        {/* Prescription Detail Modal */}
        {selected && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,20,50,0.6)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', zIndex: 100, padding: isMobile ? '16px 8px' : '32px 16px', overflowY: 'auto' }}>
            <div style={{ background: '#fff', borderRadius: 20, width: '100%', maxWidth: 680, boxShadow: '0 24px 80px rgba(21,101,192,0.25)', margin: 'auto' }}>

              {/* Modal Header */}
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: isMobile ? '16px 20px' : '20px 28px', borderBottom: '1px solid #e8edf5' }}>
                <h2 style={{ color: '#1a2744', fontWeight: 800, fontSize: isMobile ? 16 : 18, margin: 0 }}>Prescription Details</h2>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <button
                    onClick={handlePrint}
                    style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'linear-gradient(135deg, #1565c0, #1976d2)', color: '#fff', fontWeight: 700, fontSize: isMobile ? 12 : 13, padding: isMobile ? '8px 14px' : '10px 20px', borderRadius: 10, border: 'none', cursor: 'pointer', transition: 'opacity 0.2s' }}
              ...
              {/* Printable Content */}
              <div ref={printRef} style={{ padding: isMobile ? '20px' : '28px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr 1fr', gap: 20, marginBottom: 24 }}>
                  {[
                    { label: 'Doctor', main: `Dr. ${selected.doctor?.user?.name || 'Doctor'}`, sub: selected.doctor?.specialization },
                    { label: 'Patient', main: selected.patient?.name || user?.name, sub: selected.patient?.email || user?.email },
                  ].map((info, i) => (
                    <div key={i}>
                ...
                <div style={{ marginBottom: 24 }}>
                  ...
                  <div className="responsive-table-container" style={{ border: '1.5px solid #e8edf5', borderRadius: 14, overflowX: 'auto' }}>
                    <table style={{ width: '100%', minWidth: isMobile ? '500px' : 'auto', borderCollapse: 'collapse' }}>
                      <thead>
                ...
                {/* Prescription Footer */}
                <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'flex-start' : 'flex-end', justifyContent: 'space-between', borderTop: '1px solid #e8edf5', paddingTop: 16, marginTop: 8, gap: 16 }}>
                  <p style={{ fontSize: 11, color: '#90a4ae', margin: 0 }}>This is a digitally generated prescription from MedConnect.</p>
                  <div style={{ textAlign: isMobile ? 'left' : 'right', width: isMobile ? '100%' : 'auto' }}>
                    <div style={{ borderTop: '1.5px solid #1a2744', width: 160, marginBottom: 6, marginLeft: isMobile ? '0' : 'auto' }} />
                    <p style={{ fontSize: 13, fontWeight: 700, color: '#1a2744', margin: 0 }}>Dr. {selected.doctor?.user?.name}</p>
        ...
        {/* Error state */}
        {error && (
          <div style={{ background: '#ffebee', border: '1.5px solid #ef9a9a', borderRadius: 14, padding: '16px 20px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 12 }}>
            <span style={{ fontSize: 20 }}>⚠️</span>
            <div style={{ flex: 1 }}>
              <p style={{ color: '#c62828', fontWeight: 700, fontSize: 14, margin: 0 }}>Failed to load prescriptions</p>
              <p style={{ color: '#e57373', fontSize: 13, margin: '2px 0 0' }}>{error}</p>
            </div>
            <button
              onClick={fetchPrescriptions}
              style={{ background: '#c62828', color: '#fff', fontWeight: 700, fontSize: 12, padding: '8px 16px', borderRadius: 8, border: 'none', cursor: 'pointer' }}
            >
              Retry
            </button>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {prescriptions.map((rx) => (
              <div
                ...
                style={{ background: '#fff', border: '1.5px solid #e8edf5', borderRadius: 18, padding: isMobile ? '20px' : '24px 28px', cursor: 'pointer', transition: 'all 0.25s', boxShadow: '0 2px 12px rgba(21,101,192,0.05)' }}
              ...
              >
                <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'flex-start' : 'flex-start', justifyContent: 'space-between', gap: 16 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    ...
                    <div>
                      <p style={{ fontWeight: 800, color: '#1a2744', fontSize: 15, margin: '0 0 3px' }}>Dr. {rx.doctor?.user?.name || 'Doctor'}</p>
                  ...
                  <div style={{ textAlign: isMobile ? 'left' : 'right', display: 'flex', flexDirection: isMobile ? 'row' : 'column', alignItems: isMobile ? 'center' : 'flex-end', flexWrap: 'wrap', gap: 8, width: isMobile ? '100%' : 'auto', borderTop: isMobile ? '1px solid #f0f4fa' : 'none', paddingTop: isMobile ? 12 : 0, justifyContent: isMobile ? 'space-between' : 'flex-end' }}>
      ...
      <div style={{ background: '#1a2744', padding: '24px 0' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: 'center', justifyContent: 'space-between', gap: 16, textAlign: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 32, height: 32, background: '#1565c0', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
